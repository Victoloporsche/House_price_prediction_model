...
class PytorchRegression:
    def __init__(self, train, test, id_column, y_column_name, num_of_features_to_select):
        ...
        self.ytrain_df = self.train_data[[self.y_column_name]]
        self.ytrain = self.ytrain_df.values
        self.xtrain_df = self.train_data.drop([self.id_column, self.y_column_name], axis=1)
        self.xtrain = self.xtrain_df.values
        self.test_data = self.data[self.number_of_train:]
        self.id = self.test[self.id_column].reset_index()
        self.xtest_df = self.test_data.drop([self.id_column, self.y_column_name], axis=1)
    ...
    def train_model_with_torch(self):
        ...
        ytrain = torch.FloatTensor(self.ytrain)
        ...
        for i in range(epochs):
            ...
            loss = torch.sqrt(model_loss(ypred, ytrain))
    ...
    def model_prediction_pytorch(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(xtest):
                ...
                predictions.append(ytest_pred.item())
        predictions_test_df = pd.DataFrame(data=predictions, columns=[self.y_column_name])
        full_test_df = pd.concat([self.id[self.id_column], predictions_test_df], axis=1)
        return full_test_df



class NetworkModel(nn.Module):
    def __init__(self, input_features = 79, hidden_1=25, hidden_2=25, output_features=1):
        super().__init__()
